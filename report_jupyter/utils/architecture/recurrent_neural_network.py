...
class RecurrentNeuralNetwork(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int = 128, num_layers: int = 2, learning_rate: float = 1e-3):
        """
    ...
    def forward(self, x):
        # LSTM expects input shape: (batch_size, seq_length, input_dim)
    ...
    def fit(self, X_train: np.ndarray, y_train: np.ndarray,
        ...
        for epoch in progress_bar:
            ...
            plt.tight_layout()
            plt.savefig('./visualisations/recurrent_neural_network_training_plot.png')
            plt.show()
        ...
        imageio.mimsave('./visualisations/recurrent_neural_network_training.gif', frames, fps=2)
    ...
    def predict(self, X: np.ndarray, y_test: np.ndarray = None, display_metrics: bool = True) -> np.ndarray:
        ...
        if display_metrics and y_test is not None:
            ...
            plt.ylabel('True')
            plt.savefig('./visualisations/recurrent_neural_network_confusion_matrix.png')
            plt.show()
